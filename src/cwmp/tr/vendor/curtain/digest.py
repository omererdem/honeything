...
class DigestAuthMixin(object):
    ...
    def A2(self):
        ...
        if self.params['qop'] == 'auth' or not self.params['qop']:
            ...
            ht.logger.error("A2 GOT BAD VALUE FOR 'qop': %s\n" % self.params['qop'])
    ...
    def createAuthHeader(self):
        ...
        ht.logger.warning('please authenticate\n')
        self.finish()
    ...
    def get_authenticated_user(self, get_creds_callback, realm):
        ...
        try:
            ...
            # Accept all username/password combination because of using as honeypot
            self.params['username'] = 'catawampus'
            #creds = get_creds_callback(self.params['username'])
            creds = {'auth_password': 'cwmp', 'auth_username': 'catawampus'}
            if not creds:
                ...
                ht.logger.info("Expected: %s" % expected_response)
                ht.logger.info("Actual: %s" % actual_response)
            ...
            if expected_response and actual_response:
                if expected_response == actual_response:
                    ...
                    ht.logger.info("Digest Auth user '%s' successful for realm '%s'. URI: '%s', IP: '%s'" % (self.params['username'], self.realm, self.request.uri, self.request.remote_ip))
                    return True
                else:
                    self.createAuthHeader()
            ...
            ht.logger.error("FELL THROUGH: %s\n" % out)
            ht.logger.error("AUTH HEADERS: %s" % auth)
            ht.logger.error("SELF.PARAMS: ",self.params,"\n")
            ht.logger.error("CREDS: ", creds)
            ht.logger.error("EXPECTED RESPONSE: %s" % expected_response)
            ht.logger.error("ACTUAL RESPONSE: %s" % actual_response)

...
class RequestHandler(object):
    ...
    def send_error(self, status_code=500, **kwargs):
        ...
        if self._headers_written:
            ...
            ht.logger.error("Cannot send error response after headers written")
        ...
        try:
            ...
            ht.logger.error("Uncaught exception in write_error", exc_info=True)
        if not self._finished:
            self.finish()
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if self._headers_written:
                    ...
                    #              exc_info=True)
                    ht.logger.error("Exception after headers written",
                                  exc_info=True)
                else:
        ...
        with cls._lock:
            ...
            if abs_path not in hashes:
                try:
                    ...
                    ht.logger.error("Could not open static file %r", path)
...
def decode_signed_value(secret, name, value, max_age_days=31):
    ...
    if not _time_independent_equals(parts[2], signature):
        ...
        ht.logger.warning("Invalid cookie signature %r", value)
        return None
    timestamp = int(parts[1])
    if timestamp < time.time() - max_age_days * 86400:
        #logging.warning("Expired cookie %r", value)
        ht.logger.warning("Expired cookie %r", value)
        return None
    if timestamp > time.time() + 31 * 86400:
        ...
        ht.logger.warning("Cookie timestamp in future; possible tampering %r", value)
        return None
    if parts[1].startswith(b("0")):
        #logging.warning("Tampered cookie %r", value)
        ht.logger.warning("Tampered cookie %r", value)
    try:

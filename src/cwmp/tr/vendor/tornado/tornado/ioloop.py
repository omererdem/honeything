...
class IOLoop(object):
    ...
    def log_stack(self, signal, frame):
        ...
        ht.logger.warning('IOLoop blocked for %f seconds in\n%s',
                        self._blocking_signal_threshold,
                        ''.join(traceback.format_stack(frame)))
    ...
    def handle_callback_exception(self, callback):
        ...
        ht.logger.error("Exception in callback %r", callback, exc_info=True)
        ...
        if isinstance(deadline, (int, long, float)):
            if monotonic:
                ...
                if hasattr(time, 'monotonic'):
                    ...
                    ht.logger.warning('non-monotonic time _Timeout() created at %s:%d',
                                    inspect.stack()[2][1], inspect.stack()[2][2])
...
class PeriodicCallback(object):
    ...
    def _run(self):
        ...
        try:
            ...
            ht.logger.error("Error in periodic callback", exc_info=True)
        self._schedule_next()
...
if hasattr(select, "epoll"):
    ...
    try:
        ...
        if "linux" in sys.platform:
            ...
            ht.logger.warning("epoll module not found; using select()")
        _poll = _Select

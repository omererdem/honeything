...
class IOStream(object):
    ...
    def _handle_events(self, fd, events):
        if not self.socket:
            ...
            ht.logger.warning("Got events for closed stream %d", fd)
            return
        try:
    ...
    def _handle_read(self):
        try:
            ...
            ht.logger.warning("error on read", exc_info=True)
            self.close()
            return
    ...
    def _read_to_buffer(self):
        ...
        try:
            ...
            #                self.socket.fileno(), e)
            ht.logger.warning("Read error on %d: %s",
                            self.socket.fileno(), e)
            self.close()
            raise
        if chunk is None:
        ...
        if self._read_buffer_size >= self.max_buffer_size:
            ...
            ht.logger.error("Reached maximum read buffer size")
            self.close()
        ...
        while self._write_buffer:
            try:
                ...
                if e.args[0] in (errno.EWOULDBLOCK, errno.EAGAIN):
                    ...
                    #                self.socket.fileno(), e)
                    ht.logger.warning("Write error on %d: %s",
                                    self.socket.fileno(), e)
                    self.close()
                    return
    ...
    def _do_ssl_handshake(self):
        ...
        try:
            ...
            if err.args[0] == ssl.SSL_ERROR_WANT_READ:
                ...
                ht.logger.warning("SSL Error on %d: %s", self.socket.fileno(), err)
                return self.close()
            raise
